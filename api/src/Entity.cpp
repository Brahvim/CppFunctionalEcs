...
#pragma region // Entity data arrays.
    ...
    static size_t s_count = 0;      // Number of entities.
    static size_t s_free_count = 0; // Number of free entities.

    // Allocations for how many elements in each array?:
    static size_t s_allocs_count = 8;                           // Number of entities currently allocated.
    static size_t s_free_allocs_count = s_allocs_count;         // Number of entities the free list has allocations for.

    static size_t s_component_types_length = s_allocs_count;    // Component-types allocated length.
    static size_t s_components_list_length = s_allocs_count;    // Component-list allocated length.
    static size_t s_component_counts_length = s_allocs_count;   // Component-counts allocated length.

    // Free list:
    static struct ecs::entity *s_free_list = static_cast<struct ecs::entity*>(std::calloc(s_free_allocs_count, sizeof(struct ecs::entity)));

    // How many components do we have in each entity?:
...
#pragma region // API implementation.
    ...
    bool entity_is_valid(const struct ecs::entity p_entity) {
        if (p_entity.offset < 1 || p_entity.offset > s_count)
            return false;

        for (size_t i = 0; i < s_free_count; ++i)
            if (s_free_list[i].offset == p_entity.offset)
                return false;

        return true;
    }

    enum ecs::entity_error entity_create(struct ecs::entity *p_storage) {
        if (!p_storage)
    ...
    // ...Whatever:
    enum ecs::entity_error entity_destroy(const struct ecs::entity p_entity) {
        if (ecs::entity_is_valid(p_entity)) // Is the ID valid?
            return ecs::entity_error::INVALID_ENTITY;

        if (++s_free_count >= s_free_allocs_count) {
            s_free_allocs_count *= 2;
            s_free_list = static_cast<struct ecs::entity*>(std::realloc(s_free_list, s_free_allocs_count * sizeof(struct ecs::entity)));
    ...
    enum ecs::entity_error entity_ensure_allocations_for(const size_t p_count) {
    ...
    // If you get `0`, well, that's the error state.
    size_t entity_get_component_type_count(const struct ecs::entity p_entity, const ecs::component_type *p_type) {
        return 0;
    }

    // If you get `nullptr`, you know it wasn't attached. No error code needed.
    struct ecs::component* entity_get_components(const struct ecs::entity p_entity, const ecs::component_type *p_type, const size_t *p_count) {
        struct ecs::component *components = nullptr;
        const size_t max = s_component_counts[p_entity.offset];

        for (size_t i = 0; i < max; ++i)
            if (s_component_types[i] == p_type) {
                components = s_components_list[i];
                break;
            }
    ...
    enum ecs::entity_error entity_attach_component(const struct ecs::entity p_entity, const struct ecs::component *p_component) {
        if (!(ecs::entity_is_valid(p_entity) && p_component))
    ...
    enum ecs::entity_error entity_detach_component(const struct ecs::entity p_entity, const struct ecs::component_type *p_type) {
        if (ecs::entity_is_valid(p_entity))
            return ecs::entity_error::INVALID_ENTITY;
